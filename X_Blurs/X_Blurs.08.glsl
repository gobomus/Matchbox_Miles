...
uniform sampler2D adsk_results_pass1, adsk_results_pass2, adsk_results_pass3, adsk_results_pass7;
uniform int result;
...
void main(void)
{
	...
	vec4 blurred = texture2D(adsk_results_pass7, st);
	...
	if (keep_inside) {
		matte = texture2D(adsk_results_pass3, st);
	}

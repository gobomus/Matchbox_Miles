...
uniform float blur_flare;

float v_bias = 1.0;
//float h_bias = 1.0; //Blur Horizontal

//float bias = v_bias;
float bias = v_bias; //Blur Horizontal
...
vec4 gblur(sampler2D source)
{
	...
	float sigma = blur_flare * bias * strength + .001;
...
void main(void)
{
	gl_FragColor = gblur(adsk_results_pass10);
}

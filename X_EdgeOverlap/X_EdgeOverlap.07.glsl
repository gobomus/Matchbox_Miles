...
uniform sampler2D adsk_results_pass3, adsk_results_pass2;

uniform float back_blur_amount;
...
vec4 gblur(sampler2D source)
{
    ...
    float sigma = back_blur_amount * bias * strength + .001;
...
void main(void)
{
	vec4 alpha = gblur(adsk_results_pass3);
	vec4 front = gblur(adsk_results_pass2);

    gl_FragColor = vec4(front.rgb, 1.0 - alpha.a);
}

...
void main()
{
   ...
   vec2 sourceGradient = vec2(0.0);
   
   if ( inputIsGradient )
   {
      sourceGradient = texture2D( warpMap, coords ).rg;
   }
   else
   {
      sourceGradient = computeGrad( coords );
   }
   
   vec2 gradient = sourceGradient * vec2(effect/100.0);
   
   vec3 resultColor = vec3(0.0);
   
   resultColor = texture2D( front, coords + gradient ).rgb;

...
float draw_circle(vec2 v2, vec2 v3)
{
	float circle =  1.0 - smoothstep(length(v2) - smooth*smooth, length(v2), length(v3));
...
void main()
{
	...
	if (sick) {
		col += blue(sin(angle_in_degrees));
	}

	// View the angle analyze with a cc node to see the values go from 0 - 360
	//col = vec3(0.0, 0.0, angle_in_degrees);

	float circle = draw_circle(v2, v3);

	col += clamp(green(circle) * (angle_in_degrees / 360.0), 0.0, 1.0);

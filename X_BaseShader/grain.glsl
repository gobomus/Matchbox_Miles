...
void main() 
{
	...
	vec3 noise = vec3(pnoise3D(vec3(texCoord*vec2(width,height),0.0)));
  
	if (colored)
	{
		noise.g = pnoise3D(vec3(texCoord*vec2(width,height),1.0));
		noise.b = pnoise3D(vec3(texCoord*vec2(width,height),2.0));
	}
